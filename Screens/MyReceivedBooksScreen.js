...
export default class MyReceivedBooksScreen extends Component{
  ...
  getReceivedBooksList=()=>{
    ...
    .where('user_id','==',this.state.userId).where("book_status", '==','received')
    .onSnapshot((snapshot)=>{
  ...
  renderItem=({item, i})=>(
    <ListItem key={i} title={item.book_name} bottomDivider
    titleStyle={{ color: 'black', fontWeight: 'bold' }}
    subtitle={item.book_status[0].toUpperCase() + item.book_status.slice(1)} />
  )
